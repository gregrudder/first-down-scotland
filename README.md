...
- **`/this-week`** : this week’s NFL games from ESPN’s public scoreboard, times in `Europe/London`, plus a **Your Sunday** card for the team saved in the browser (`fds-team`). Each fixture has a short beginner preview or a post-match report (score + snippet + outbound link; “report coming” until a feed publishes one). Fair-use summaries only. Live and finished cards list **touchdown scorers** under the scoreline when ESPN’s game summary has scoring plays, plus **highlights**: an in-app YouTube embed when `YOUTUBE_API_KEY` finds an official clip that YouTube marks embeddable, otherwise a link-out to that video or NFL YouTube search (the NFL often blocks in-app playback). **Spoiler-free** (Morning-after) is a toggle on This week and Scores: it hides scores, TD scorers and recaps, kept in `fds-spoiler-free`. Highlights stay on the card behind a branded “no spoiler” poster until you confirm. Kick-off, TV and teams stay visible. Also UK kick-off, what to watch for, a Learn tie-in, optional ESPN QB snapshot, and a Scottish pub meetup hint. The full slate stays free. Tabs also lead to live scores, standings, and Rookie Watch.
...
| `YOUTUBE_API_KEY` | Optional | Server-only [YouTube Data API v3](https://developers.google.com/youtube/v3) key. Never `NEXT_PUBLIC_*`. When set, live and final cards resolve an official NFL / club clip. If YouTube’s `status.embeddable` is true we iframe it (`youtube-nocookie.com`); if the NFL blocks embedding we link to that `watch` URL. Unset / quota / no match falls back to NFL YouTube search. |
...
8. **Spoiler-free** is a client preference (`localStorage` key `fds-spoiler-free`, values `on` / `off`). A before-paint script sets `data-spoiler-free="on"` on `<html>` so scores do not flash. CSS then hides `.fds-spoiler` (scoreline, TD list, recaps, this-week unusual-finals, highlight player / links). Highlight tiles stay visible as `.fds-spoiler-safe` branded posters (`/highlights-spoiler-free.svg`) so a YouTube thumbnail never flashes. Tapping asks “This may reveal the result”; confirm shows that game’s embed or link-out and leaves the rest of the slate spoiler-free. The Sunday card’s “what to watch” line does not include the final score. Per-game “Show result” is not in this MVP.
9. **Highlights** on live and finished cards call `GET /api/highlights` (server-only). If `YOUTUBE_API_KEY` is set it searches the NFL channel (then a wider official-club filter) and checks `videos.list` `status.embeddable`. The JSON is **only** `{ videoId, embeddable, watchUrl, searchUrl }` — never a YouTube title or thumbnail. Embeddable clips play in-app (`youtube-nocookie.com`); the NFL often marks game packages not embeddable, so we link out to that watch URL instead. No key, no match, or an API error falls back to `youtube.com/@NFL/search?query=…`. We do not scrape YouTube. Club YouTube on team pages is still a link-out. The daily Cron also busts the `highlights` tag.

### YouTube Data API key on Vercel

1. In [Google Cloud](https://console.cloud.google.com/), enable **YouTube Data API v3** and create an API key. Restrict the key to that API.
2. Vercel → the project → **Settings** → **Environment Variables**.
   - Name: `YOUTUBE_API_KEY`
   - Value: the key
   - Environment: **Production** and **Preview**
   - Leave **Sensitive** on. Do not tick “Automatically expose to the browser”.
3. **Redeploy** so the new variable is picked up (Deployments → current Production → Redeploy, or push a new commit).
4. The key is read as `process.env.YOUTUBE_API_KEY` on the server only. It is not in this repo.

Without the variable, highlights still work as NFL YouTube search links.
...
3. Add `CRON_SECRET` (and optionally `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_DISCORD_INVITE`, `NEXT_PUBLIC_CONTACT_EMAIL`, and `YOUTUBE_API_KEY`) under Project Settings → Environment Variables. `YOUTUBE_API_KEY` is server-only — see **YouTube Data API key on Vercel** above. Leave it unset and highlights still work as NFL YouTube search links.
