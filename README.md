...
- **`/mini-games`** : separate browser games — beginner quizzes (rules, Who am I?, down-and-distance, rivalry match-up) plus ten fun fan quizzes (Super Bowls, catches, jersey numbers, late-night survival, and the rest). Not part of the lesson path.
...
5. **Who Owns Scotland (live).** A Scottish town is owned by the leading NFL team only after it hits `FAN_MAP_PRIVACY_THRESHOLD` (default 3). Below that it stays uncoloured and does not count. A create/update that changes the leader writes a `fan_map_flips` row (survives reloads). `/fan-map#who-owns-scotland` has recent territory changes plus a towns-owned board (towns first, fans secondary). Scheme Battles on `/fan-map#scheme-battles` adds the UK table.
...
| `/mini-games` | Mini Games hub: beginner games + fun fan quizzes (separate from lessons) |
