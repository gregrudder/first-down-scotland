...
8. **Spoiler-free** is a client preference (`localStorage` key `fds-spoiler-free`, values `on` / `off`). A before-paint script sets `data-spoiler-free="on"` on `<html>` so scores do not flash. CSS then hides `.fds-spoiler` (scoreline, TD list, recaps, this-week unusual-finals, highlight links). Per-game “Show result” is not in this MVP.
