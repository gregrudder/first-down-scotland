...
- **`/late-night-diary`** : UK planner for games that kick off **at 9pm UK or later, or overnight before 5am** (Sunday 4pm ET window plus Thursday / Sunday / Monday night in the US). Times in `Europe/London`, with a US Eastern clock for context and a “Sunday night into Monday morning” style label. Filter or highlight by the team saved as `fds-team`. **Times only** — no scores, even if spoiler-free is off. Uses the same ESPN scoreboard as This week, plus the previous week and the next two slates.
...
5. `/` and `/this-week` also set `export const revalidate = 300`. `/late-night-diary` uses the same cache, then asks ESPN for the previous week and the next two slates (`year` / `seasontype` / `week`) so a planner can see more than the current Sunday card.
